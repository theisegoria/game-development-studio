...
APP_NAME="Anvil"
DISPLAY_NAME="Anvil"
BUNDLE_ID="com.theisegoria.Anvil"
MIN_SYSTEM_VERSION="26.0"

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
PACKAGE_DIR="$ROOT_DIR/apps/macos/Anvil"
ICON_SOURCE="$PACKAGE_DIR/Resources/AnvilIcon.png"
ICON_COMPILED="$PACKAGE_DIR/Resources/AnvilIcon.icns"
...
if [[ ! -f "$ICON_COMPILED" || "$ICON_SOURCE" -nt "$ICON_COMPILED" ]]; then
  "$PACKAGE_DIR/Tools/make-icns.sh" >/dev/null
fi
if [[ ! -f "$ICON_SOURCE" || ! -f "$ICON_COMPILED" ]]; then
  echo "could not generate the app icon master or compiled ICNS asset" >&2
  exit 1
fi
...
# after it is written. codesign --strict refuses those as "detritus", so the clear has
# to happen immediately before the verify rather than earlier in the build, and a single
# retry covers the case where the provider wins the race anyway.
verify_signature() {
  /usr/bin/xattr -cr "$APP_BUNDLE"
  /usr/bin/codesign --verify --deep --strict "$APP_BUNDLE"
}
verify_signature || {
  echo "signature verification hit extended-attribute detritus; clearing and retrying" >&2
  verify_signature
}
